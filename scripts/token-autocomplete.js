...
var TokenAutocomplete = /** @class */ (function () {
    function TokenAutocomplete(options, logic) {
        this.logic = logic; // 248 part
            ...
            if (event.which == me.KEY_ENTER || event.keyCode == me.KEY_ENTER) {
                ...
                if (highlightedSuggestion !== null) {
                    if (highlightedSuggestion.classList.contains('token-autocomplete-suggestion-active')) {
                        ...
                        let tag = '';
                        for (let i = 0; i < highlightedSuggestion.childNodes.length; i++) {
                            if (highlightedSuggestion.childNodes[i].nodeType === 3) {
                                tag = highlightedSuggestion.childNodes[i].data;
                            }
                        }
                        me.select.addToken(highlightedSuggestion.getAttribute('data-value'), tag);
                    }
                }
                else {
    ...
    // 248 Part
    TokenAutocomplete.prototype.checkTags = (me) => {
        me.logic.checkTags();
    }
